...
const ChapterList = ({ course, chapterVideoCache, refreshData, courseId }) => {
    ...
    <div className='border mt-5 ml-4 rounded-lg shadow-sm p-4'>
       ...
       <div className="space-y-4">
          {chapters.map((chapter,index)=>{
              const cachedChapter = cachedVideos[index]
              const chapterId = cachedChapter?.chapterId || `${courseId}-${index}`
              return (
                <Link
                  key={index}
